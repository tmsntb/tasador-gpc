export default async function handler(req, res) {
      ...
      if (ghRes.ok) {
        ...
        if (data.text && data.text.length > 100) {
          ...
          while (true) {
            ...
            const mIdx = chunk.indexOf(modelUpper);
  ...
  const prompt = 'Buscá precios de referencia en Argentina para: ' + vehiculo + '\n' +
    (infoautoContext ? '\nDATOS INFOAUTO DEL PDF (' + infoautoLabel + ') — extraé precios exactos, ignorá sección de motos:\n' + infoautoContext + '\n' : '') +
    '\nTAREA: Buscá en Google y devolvé resultados reales:\n' +
    '1. Publicaciones de ' + vehiculo + ' en MercadoLibre Argentina (autos.mercadolibre.com.ar) — 5 a 8 publicaciones con precio, km, año y URL\n' +
    '2. Publicaciones de ' + vehiculo + ' en Rosario Garage (rosariogarage.com)\n' +
    (infoautoLabel ? '3. Infoauto: usá datos del PDF de arriba' : '3. Precio referencia Infoauto de infoauto.com.ar') + '\n' +
  ...
  try {
    // Use gemini-2.0-flash-exp with grounding
    ...
    if (!geminiRes.ok) {
      console.error('Gemini API error:', JSON.stringify(geminiData).substring(0, 500));
    ...
    } catch (e) { console.error('Parse error:', e.message, text.substring(0, 300)); }
